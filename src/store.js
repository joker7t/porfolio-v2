...
if (window.navigator.userAgent.includes('Chrome') && reactRedux) {
	store = createStore(rootProducer, initialState, compose(applyMiddleware(...middleware), reactRedux));
} else {
	store = createStore(rootProducer, initialState, compose(applyMiddleware(...middleware)));
}
